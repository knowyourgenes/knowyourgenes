...
.kyg-page .nav__link:hover,
.kyg-page .nav__item.is-open .nav__link {
  color: var(--c-teal);
  background: rgba(14, 77, 75, 0.07);
}
.kyg-page .nav__caret {
  width: 7px;
  height: 7px;
...
.kyg-page .nav__item.is-open .nav__caret {
...
.kyg-page .nav__burger span::before,
.kyg-page .nav__burger span::after {
  content: '';
  position: absolute;
  left: 0;
  right: 0;
...
.kyg-page .megamenu {
  ...
  box-shadow: 0 36px 80px rgba(45, 32, 18, 0.1);
  opacity: 0;
  visibility: hidden;
...
.kyg-page .nav__item.is-open .megamenu {
  opacity: 1;
  visibility: visible;
...
.kyg-page .megamenu__sub {
  font-size: 14px;
  color: var(--ink-3);
  max-width: 380px;
}

/* Mega card - imagery dominant, hover reveals more */
.kyg-page .mm-card {
  position: relative;
  display: block;
...
.kyg-page .mm-card:hover {
  transform: translateY(-4px);
  box-shadow: 0 22px 50px rgba(45, 32, 18, 0.18);
}
.kyg-page .mm-card__img {
  position: absolute;
  inset: 0;
  width: 100%;
  height: 100%;
  object-fit: cover;
  transition:
    transform 1.2s var(--e-out),
    filter 0.6s var(--e-out);
}
.kyg-page .mm-card:hover .mm-card__img {
  transform: scale(1.08);
  filter: brightness(0.92);
}
.kyg-page .mm-card__shade {
  position: absolute;
  inset: 0;
  background: linear-gradient(180deg, rgba(31, 26, 20, 0) 35%, rgba(31, 26, 20, 0.78) 100%);
  transition: opacity 0.6s var(--e-out);
}
.kyg-page .mm-card:hover .mm-card__shade {
  opacity: 0.92;
}
.kyg-page .mm-card__body {
  position: absolute;
  left: 18px;
  right: 18px;
  bottom: 18px;
...
.kyg-page .mm-card__desc {
  ...
  transition:
    ...
    max-height 0.5s var(--e-out),
    margin-top 0.5s var(--e-out);
}
.kyg-page .mm-card:hover .mm-card__desc {
  opacity: 0.92;
  max-height: 80px;
  margin-top: 8px;
}
.kyg-page .mm-card__arrow {
  position: absolute;
  right: 18px;
  top: 18px;
  width: 34px;
...
.kyg-page .mm-card:hover .mm-card__arrow {
  background: var(--c-teal-light);
  color: #fff;
  transform: rotate(-45deg);
}
...
.kyg-page .footer::before {
  content: '';
  position: absolute;
  inset: 0;
  background: radial-gradient(40% 60% at 90% 0%, rgba(37, 181, 171, 0.12), transparent 60%);
}
...
.kyg-page .footer__newsletter input::placeholder {
...
.kyg-page .footer__newsletter button:hover {
  background: var(--c-teal-light);
  color: #fff;
}
...
.kyg-page .footer__bottom a {
  transition: color 0.3s var(--e-out);
}
.kyg-page .footer__bottom a:hover {
  color: var(--c-cream);
}
...
.kyg-page .footer__brandline span {
  color: var(--c-peach-2);
  margin: 0 4px;
}
.kyg-page .footer__socials {
  display: flex;
  gap: 18px;
}
      ...
      <footer className="footer">
        <div className="container container--wide footer__inner">
          <div className="footer__grid">
            ...
            {/* Left stack of nav cols. display:contents on desktop makes the
                children become direct grid items of footer__grid (preserving
                the 5-col layout); at <=880px it switches to flex-column so the
                two col-groups stack continuously, with no gap from grid row
                alignment. See KYG_POLISH_CSS. */}
            ...
            {/* Right stack: Care + Company. Same trick as the left wrapper. */}
            <div className="footer__nav-col">
              <div>
