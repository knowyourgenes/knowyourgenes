...
import type { Bundle, Category, SidebarLink } from '../data';
import { scrollToHashWithOffset, useScrollSpy } from '../_hooks';
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex items-center gap-[10px] mb-[26px]', collapsed ? 'justify-center' : 'justify-between')}>
        <span className={cn('text-[11.5px] font-bold tracking-[0.16em] uppercase text-[var(--acc-700)] whitespace-nowrap', titleCls)}>{category.sidebarEyebrow}</span>
        <button
          type="button"
      ...
      {category.sidebar.map((group) => (
        <div key={group.title} className="mb-[26px]">
          <div className={cn('text-[11.5px] font-bold tracking-[0.15em] uppercase text-[var(--ink-3)] px-[12px] mb-[12px] whitespace-nowrap', titleCls)}>{group.title}</div>

          {group.kind === 'links'
            ? group.links.map((link) => <LinkRow key={link.name} link={link} active={isActive(link.href)} collapsed={collapsed} textCls={textCls} dotCls={dotCls} />)
            : group.cards.map((card) => <CardRow key={card.name} card={card} collapsed={collapsed} textCls={textCls} dotCls={dotCls} />)}
        </div>
      ))}
    </aside>
  );
...
function LinkRow({ link, active, collapsed, textCls, dotCls }: { link: SidebarLink; active: boolean; collapsed: boolean; textCls: string; dotCls: string }) {
  ...
  const inner = (
    <>
      <span className={cn('w-[30px] h-[30px] rounded-[9px] mx-auto items-center justify-center font-bold text-[13px]', active ? 'bg-[var(--acc-500)] text-white' : 'bg-[var(--acc-50)] text-[var(--acc-700)]', dotCls)}>{link.dot}</span>
      <span className={cn('text-[15.5px] font-semibold tracking-[-0.01em] whitespace-nowrap', active ? 'text-[var(--acc-700)]' : 'text-[var(--ink-1)]', textCls)}>{link.name}</span>
      {link.meta ? <span className={cn('text-[13px] text-[var(--ink-3)] mt-[2px] whitespace-nowrap', textCls)}>{link.meta}</span> : null}
    </>
  );
...
function CardRow({ card, collapsed, textCls, dotCls }: { card: Bundle; collapsed: boolean; textCls: string; dotCls: string }) {
  ...
  return (
    ...
    >
      <span className={cn('w-[30px] h-[30px] rounded-[9px] mx-auto items-center justify-center font-bold text-[13px]', t.dotBg, t.dotFg, dotCls)}>{card.dot}</span>
      <span className={cn('block text-[10.5px] font-bold tracking-[0.13em] uppercase mb-[5px] whitespace-nowrap', t.accent, textCls)}>{card.tag}</span>
      <span className={cn('block text-[15px] font-semibold tracking-[-0.01em] whitespace-nowrap', t.name, textCls)}>{card.name}</span>
