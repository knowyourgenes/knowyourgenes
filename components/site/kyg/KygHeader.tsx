...
/**
 * Shared KYG warm-modern header (mega-menu nav). Self-contained: design tokens
 * are applied inline on the <header>, so it renders identically in any scope.
 * Used by the landing page and the test-detail pages.
 */
export default function KygHeader() {
  ...
  return (
    <header
      ...
      className={cn(
        ...
        scrolled ? 'bg-[rgba(250,246,239,.88)] border-[var(--ink-line)]' : 'bg-[rgba(250,246,239,.65)] border-transparent',
      )}
    >
      <div className="w-full max-w-[1280px] mx-auto px-[var(--gutter)] flex items-center justify-between h-16 gap-[32px]">
        ...
        <nav ref={linksRef} className="flex items-center gap-[2px] flex-1 justify-center max-[980px]:hidden" aria-label="Main">
          {NAV_MENUS.map((menu) => {
                ...
                >
                  <div className="max-w-[1280px] mx-auto pt-[44px] px-[var(--gutter)] pb-[56px] grid grid-cols-4 gap-[22px] max-[1180px]:grid-cols-2">
                    <div className="col-span-full flex items-end justify-between pb-[18px] mb-[8px] border-b border-[var(--ink-line)]">
                      <div className="text-[13px] tracking-[0.22em] uppercase font-semibold text-[var(--ink-3)]">{menu.title}</div>
                      <div className="text-[14px] text-[var(--ink-3)] max-w-[380px] max-[680px]:hidden">{menu.subtitle}</div>
                    </div>
                    {menu.cards.map((card) => (
                      <a
                        key={card.title}
                      ...
                      >
                        ...
                        <img src={card.image} alt={card.imageAlt} loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-[transform,filter] duration-[1200ms] ease-[var(--e-out)] group-hover:scale-[1.08] group-hover:brightness-[.92]" />
                        ...
                        <div className="absolute left-[18px] right-[18px] bottom-[18px] text-white">
                          <div className="text-[10.5px] tracking-[0.22em] uppercase opacity-85 font-semibold inline-flex items-center gap-2 py-[5px] px-[10px] bg-white/[0.16] backdrop-blur-[10px] rounded-full">{card.kicker}</div>
                          <div className="text-[21px] font-semibold leading-[1.12] mt-[12px] tracking-[-0.015em]">{card.title}</div>
                          <div className="text-[13px] leading-[1.45] mt-[6px] max-h-0 overflow-hidden opacity-0 transition-[opacity,max-height,margin-top] duration-500 ease-[var(--e-out)] group-hover:opacity-[.92] group-hover:max-h-[80px] group-hover:mt-[8px]">{card.desc}</div>
                        </div>
                      </a>
                    ))}
                  </div>
          ...
          {NAV_LINKS.map((link) => (
            <a key={link.label} className={NAV_LINK} href={link.href}>
              {link.label}
            </a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-[10px] shrink-0">
          {SHOW_NAV_CTA &&
            (user ? (
              <>
                {user.role === 'ADMIN' ? (
                  <Link href="/admin/dashboard" className={cn(BTN_BASE, 'bg-white text-[var(--ink-1)] border-[var(--ink-line)] max-[980px]:hidden')}>
                    <LayoutDashboard className="w-4 h-4" />
                    Dashboard
                  </Link>
                ) : null}
                <Link href="#wellness" className={cn(BTN_BASE, 'bg-[var(--ink-1)] text-[var(--cream)] hover:bg-[var(--teal)] hover:-translate-y-[3px]')}>
                  Order Kit <ArrowRight />
                </Link>
                <UserNav name={user.name ?? user.email ?? 'User'} email={user.email ?? ''} role={user.role} image={user.image ?? null} />
              </>
            ) : status !== 'loading' ? (
              <>
                <Link href="#wellness" className={cn(BTN_BASE, 'bg-[var(--ink-1)] text-[var(--cream)] hover:bg-[var(--teal)] hover:-translate-y-[3px]')}>
                ...
                <Link href={`/login?from=${encodeURIComponent(pathname || '/')}`} className={cn(BTN_BASE, 'bg-white text-[var(--ink-1)] border-[var(--ink-line)] max-[980px]:hidden')}>
                  Sign in
                </Link>
              </>
            ) : null)}
      ...
      {mobileOpen && (
        ...
        {NAV_MENUS.map((menu) => (
          <div key={menu.key}>
            <div className="text-[11px] tracking-[0.2em] uppercase font-bold text-[var(--ink-3)] mb-[10px]">{menu.label}</div>
            <ul className="flex flex-col gap-[2px]">
              {menu.cards.map((card) => (
                <li key={card.title}>
                  <a href={card.href} onClick={() => setMobileOpen(false)} className="block py-[9px] text-[15px] font-medium text-[var(--ink-1)] hover:text-[var(--teal)]">
                    {card.title}
                  </a>
                </li>
              ))}
        ...
        <div className="border-t border-[var(--ink-line)] pt-[16px] flex flex-col gap-[2px]">
          {NAV_LINKS.map((link) => (
            <a key={link.label} href={link.href} onClick={() => setMobileOpen(false)} className="block py-[9px] text-[15px] font-semibold text-[var(--ink-1)] hover:text-[var(--teal)]">
              {link.label}
            </a>
          ))}
            </div>
