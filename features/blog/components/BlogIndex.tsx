...
function MetaLine({ post }: { post: BlogListItem }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[13px] text-[var(--ink-3)]">
      {bits.map((b, i) => (
        <span key={i} className="inline-flex items-center gap-2">
          {i > 0 && <span className="text-[var(--ink-line)]">•</span>}
          {b}
        </span>
      ))}
      {post.readMinutes ? (
...
function CategoryBadge({ category }: { category?: string }) {
  return (
    <span className="inline-flex w-fit items-center rounded-full bg-[var(--acc-50)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--teal)]">
...
function PostCard({ post }: { post: BlogListItem }) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group flex flex-col overflow-hidden rounded-[var(--r-md)] border border-[var(--ink-line)] bg-white shadow-[var(--sh-1)] transition-[transform,box-shadow] duration-500 ease-[var(--e-out)] hover:-translate-y-1 hover:shadow-[var(--sh-2)]"
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-[var(--cream-2)]">
        {img ? (
          ...
          <img
            ...
            loading="lazy"
            className="absolute inset-0 h-full w-full object-cover transition-transform duration-[900ms] ease-[var(--e-out)] group-hover:scale-[1.05]"
          />
        ) : null}
      </div>
      <div className="flex flex-1 flex-col gap-3 p-6">
        <CategoryBadge category={post.category} />
        <h3 className="text-[19px] font-semibold leading-[1.3] tracking-[-0.015em] text-[var(--ink-1)] transition-colors group-hover:text-[var(--teal)]">
          {post.title}
        </h3>
        {post.excerpt ? (
...
function FeaturedCard({ post }: { post: BlogListItem }) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group grid overflow-hidden rounded-[var(--r-lg)] border border-[var(--ink-line)] bg-white shadow-[var(--sh-1)] transition-[transform,box-shadow] duration-500 ease-[var(--e-out)] hover:shadow-[var(--sh-2)] md:grid-cols-2"
    >
      <div className="relative aspect-[16/11] overflow-hidden bg-[var(--cream-2)] md:aspect-auto md:min-h-[340px]">
        {img ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={img}
            alt={post.heroImage?.alt ?? post.title}
            className="absolute inset-0 h-full w-full object-cover transition-transform duration-[900ms] ease-[var(--e-out)] group-hover:scale-[1.04]"
          />
        ) : null}
      </div>
      <div className="flex flex-col justify-center gap-4 p-8 md:p-10">
        ...
        {post.excerpt ? (
          <p className="max-w-[46ch] text-[16px] leading-[1.7] text-[var(--ink-3)]">{post.excerpt}</p>
        ) : null}
        <MetaLine post={post} />
      </div>
    </Link>
  ...
  const pill = (label: string, href: string, active: boolean) => (
    <Link
      ...
      className={
        ...
        (active
          ? 'bg-[var(--ink-1)] text-[var(--cream)]'
          : 'bg-white text-[var(--ink-2)] border border-[var(--ink-line)] hover:border-[var(--teal)] hover:text-[var(--teal)]')
      }
    >
      {label}
    </Link>
  ...
  return (
    <div style={CHROME_VARS} className="min-h-screen bg-[var(--cream)] text-[var(--ink-1)]">
      <div className="mx-auto max-w-[1180px] px-[var(--gutter)] py-[clamp(40px,6vw,80px)]">
        ...
        {filtered.length === 0 ? (
          <div className="mt-16 rounded-[var(--r-md)] border border-dashed border-[var(--ink-line)] bg-white/60 px-6 py-16 text-center">
            <p className="text-[18px] font-medium text-[var(--ink-1)]">No articles here yet</p>
