...
export default function BlogArticle({ post }: { post: BlogFullPost }) {
  ...
  // Headings power the sticky quick-nav; ids (keyed by block _key) are stamped
  // onto every rendered h2/h3 so anchors resolve. The FAQ section is stamped but
  // kept out of the visible nav.
  const headings = extractHeadings(post.body);
  const headingIds = new Map(headings.map((h) => [h.key, h.id]));
  const components = buildPortableComponents(headingIds);
  const tocHeadings = headings.filter((h) => !h.faq);
        ...
        >
          <article>
            <header>
              ...
              {post.excerpt ? (
                <p className="mt-4 max-w-[70ch] text-[18.5px] leading-[1.6] text-(--ink-3)">{post.excerpt}</p>
              ) : null}
