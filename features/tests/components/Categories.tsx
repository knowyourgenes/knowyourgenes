...
const countChip =
  'self-start rounded-full bg-eden/[0.07] px-[11px] py-[5px] text-[11.5px] font-bold uppercase tracking-[0.08em] text-eden';
          ...
          <div className="mt-10 grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-[22px]">
            {categories.map((c) => (
              <Link
                ...
                key={c.slug}
                className={`group relative flex flex-col overflow-hidden rounded-3xl border border-zeus/[0.09] bg-white shadow-kyg-card ${cardHover}`}
              >
...
function FeatureCard({ category }: { category: TestCategory }) {
  ...
  return (
    <Link
      ...
      className={`group relative mt-10 grid overflow-hidden rounded-3xl border border-zeus/[0.09] bg-white shadow-kyg-card md:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] ${cardHover}`}
    >
