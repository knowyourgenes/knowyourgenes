...
const BTN_BASE =
  ...
  'duration-300 ease-(--e-out) [&_svg]:w-4 [&_svg]:h-4 [&_svg]:transition-transform [&_svg]:duration-500';

const BTN_VARIANTS = {
  accent:
    'bg-(--acc-500) text-white shadow-[0_10px_26px_-8px_rgba(31,107,67,.5)] hover:bg-(--acc-700) ' +
    'hover:-translate-y-[3px] hover:shadow-[0_18px_38px_-10px_rgba(31,107,67,.55)] hover:[&_svg:last-child]:translate-x-1',
  dark: 'bg-(--ink-1) text-(--cream) shadow-[0_10px_26px_-8px_rgba(31,26,20,.4)] hover:bg-(--teal) hover:-translate-y-[3px]',
  primary:
    'bg-(--ink-1) text-(--cream) shadow-[0_10px_28px_rgba(31,26,20,.18)] hover:bg-(--teal) ' +
    'hover:-translate-y-[3px] hover:shadow-[0_18px_44px_rgba(14,77,75,.32)] hover:[&_svg:last-child]:translate-x-1',
  ghost:
    'bg-white text-(--ink-1) border-(--ink-line) hover:border-[rgba(31,26,20,.2)] hover:-translate-y-[3px] hover:shadow-(--sh-2)',
  light: 'bg-white text-(--acc-700) hover:-translate-y-[3px] hover:shadow-[0_14px_30px_rgba(0,0,0,.2)]',
} as const;
...
const BADGE_STATUS = {
  ...
  risk: 'bg-(--st-risk-bg) text-(--st-risk-fg)',
} as const;
...
export function Eyebrow({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <span
      className={cn(
        'inline-flex items-center gap-3 text-[13px] font-bold tracking-[0.16em] uppercase text-(--acc-700)',
